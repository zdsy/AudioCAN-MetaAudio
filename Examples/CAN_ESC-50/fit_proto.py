"""
The file contains the main fit function for the meta-learner along with the
...
def fit(device, learner, optimiser, scheduler, loss_fn, dataloaders, prep_batch_fns,
    ...
    Args:
        ...
        prep_batch_fns (list of Function): Th prep batch functions to be used
        fit_functions (list of Function): The actual meta batch fitting function to be called
    ...
    for epoch in main_loop:
        for batch_index, batch in enumerate(trainLoader):
            ...
            if round(train_post, 2) == float(1/params['base']['n_way']):
                break_counter += 1
    ...
    # best_model_path = '/home/ids/xzhuang/MetaAudio-A-Few-Shot-Audio-Classification-Benchmark/Examples/CAN_ESC-50/results/CAN_ESC50_5-1_k=04resnet_global_1_runs/CAN_ESC50_5-1_k=04resnet_global_1_runs_5000_seed_554/best_val_model__26_01__23_21.pt'
...
def validation_step_fixed(valLoader, model, optimiser, prep_batch, fit_function, loss_fn, params, **meta_func_kwargs):
    ...
    for batch_index, batch in enumerate(valLoader):
        # Prep batch and move to GPU
        x, y, _ = prep_batch(batch, params['training']['train_batch_size'])
