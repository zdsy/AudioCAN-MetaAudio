...
class Model(nn.Module):
    def __init__(self, scale_cls, iter_num_prob=35.0/75, num_classes=5):
    ...
    def forward(self, xtrain, xtest, ytrain, ytest):
        ...
        # print(ftrain.shape)
        ftrain, ftest, atrain, atest = self.cam(ftrain, ftest)
        ftrain = ftrain.mean(-2)
        ftrain = ftrain.mean(-1)

        if not self.training:
            return self.test(ftrain, ftest, atrain, atest)
        ...
        # ftest = ftest.view(batch_size * num_test, self.nFeat, -1, 160)
        # ftest = ftest.view(batch_size * num_test, self.nFeat, 8, 10)
        ftest = ftest.view(batch_size * num_test, self.nFeat, 8, 9)
        # ftest = ftest.view(batch_size * num_test, self.nFeat, 1, 10)
        # ftest = ftest.view(batch_size * num_test, -1, 6, 6)
        # print(ftest.shape)
        ytest = self.classifier(ftest)
        return ytest, cls_scores, atrain, atest
        # return cls_scores
...
if __name__ == '__main__':
    ...
    x1 = torch.rand(5, 5, 1, 128, 431)
    x2 = torch.rand(5, 1, 1, 128, 431)
    y1 = torch.rand(5, 5, 5)
    y2 = torch.rand(5, 1, 5)
