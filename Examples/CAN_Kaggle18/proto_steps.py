...
def proto_step_fixed(device, model, optimiser, loss_fn, x, y, pid, k_shot, n_way, q_queries,
    ...
    for idx in range(x.shape[0]):
        ...
        meta_batch = x[idx]
        y_batch = y[idx]
        pid_batch = pid[idx][:5].unsqueeze(0)

        # print(meta_batch.shape)
        # print(y_batch)
        # print(pid_batch)

        s = meta_batch[:k_shot*n_way].view(-1, 1, 1, meta_batch.shape[-2], meta_batch.shape[-1])
        q = meta_batch[k_shot*n_way:].view(-1, 1, meta_batch.shape[-2], meta_batch.shape[-1])
        ...
        slabel_oh = one_hot(slabel).double().to(device)
        qlabel_oh = one_hot(qlabel).double().to(device)

        ytest, cls_scores, _, _ = model(s, q, slabel_oh, qlabel_oh)
        ytest_ssl, cls_scores_ssl, a1, a2 = model(s, q_ssl, slabel_oh, qlabel_oh)
...
def proto_step_var(model, optimiser, loss_fn, x_support, x_query, q_num, y,
    ...
    for idx in range(x_support.shape[0]):
        ...
        s_v = x_task_train.view(1, -1, 1, x_task_train.shape[-2], x_task_train.shape[-1])
        q_v = x_task_val[sampled_indices].unsqueeze(0)
