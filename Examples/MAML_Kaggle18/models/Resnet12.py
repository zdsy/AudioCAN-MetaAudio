...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, kernel=3):
        ...
        # num_logits = int(128 * floor_power(128, 3, 4) * floor_power(157, 3, 4))
        num_logits = 128
        self.logits = nn.Sequential(nn.Dropout(p=0.3), nn.BatchNorm1d(num_logits, eps=1e-05, momentum=0.1, affine=True),
                                    nn.Linear(in_features=num_logits, out_features=128))
        self.gap = nn.AdaptiveAvgPool2d((1, 1))

        # self.layer1 = self._make_layer(block, 128, layers[0], stride=(2, 1))
        ...
        self.nFeat = 128 * block.expansion
    ...
    def forward(self, x):
        ...
        # turn to logits for non-CAN model
        x = self.gap(x)
        x = x.view(x.size(0), -1)
        # print(x.shape)
        x = self.logits(x)
