...
# HELPER FUNCTIONS
##############################################################################
def create_class_folders(folder_list, sorted_train, main_dir):
...
def main(main_dir):
    ...
    os.makedirs(os.path.join(main_dir, sorted_train), exist_ok=True)
    ...
    big_df = pd.concat([test_df, train_df])
    ...
    folder_paths = create_class_folders(folders, sorted_train, main_dir)
